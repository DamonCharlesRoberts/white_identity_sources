...
incomerace12 <- read_csv('data/2012_acs_census_income_by_race/2012_acs_census_income_by_race.csv') %>%
                  slice(-1)
urban12 <- read_dta('data/anes-2012/6_urbany_anes_2012_updated.dta')

#### Cleaning the Data ####
incomerace12 <- incomerace12 %>%
...
incomerace12[] <- lapply(incomerace12, function(x) as.numeric(as.character(x)))
...
#### Merge the data ####
urban12$district <- as.numeric(urban12$district)
incomerace12$district <- as.numeric(incomerace12$district)
income12_combined <- left_join(urban12, incomerace12, by = "district")
...
income12_combined <- income12_combined %>%
                      rename(fulldistrictx3 = fulldistrict.x, 
                            fulldistricty2 = fulldistrict.y)
write_dta(income12_combined, "data/anes-2012/7_raceincome_anes_2012_updated.dta")
